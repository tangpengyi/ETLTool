  ...
  export default {
    ...
    methods: {
      connection(){
        ...
        this.axios.post('http://localhost:8080/test/socketConnection',
          {
      ...
      disconnect(){
        console.info("断开连接");
        this.axios.get('http://localhost:8080/test/disconnectSocket')
          .then((response) => {
            const result = response.data;
            if (result.code == '0') {
              console.log("失败")
      ...
      sendMessage(){
        ...
        this.axios.post('http://localhost:8080/test/socketSendMsg',
          {
            reqFrom: this.reqFrom
          }
        )
